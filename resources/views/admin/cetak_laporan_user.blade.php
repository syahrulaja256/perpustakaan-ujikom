...
<body>
    ...
    <p style="margin-bottom:10px; font-size:11px;">Total: <strong>{{ count($users) }}</strong> user</p>
    <table>
        <thead>
            <tr>
                <th>No</th><th>Nama</th><th>Email</th><th>Tanggal Daftar</th>
            </tr>
        </thead>
        <tbody>
            @foreach($users as $i => $u)
            <tr>
                <td>{{ $i+1 }}</td><td>{{ $u->name }}</td><td>{{ $u->email }}</td><td>{{ $u->created_at ? $u->created_at->format('d/m/Y') : '-' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
