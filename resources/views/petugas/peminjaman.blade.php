                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    ...
                                    @elseif($p->status == 'Dikonfirmasi')
                                        @if ($p->dikonfirmasi_oleh == 'admin')
                                            <span class="inline-flex items-center gap-1 bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg text-xs font-semibold">
                                        ...
                                        @else
                                            <span class="inline-flex items-center gap-1 bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg text-xs font-semibold">
                                                <i class="fa-solid fa-check text-[10px]"></i> Petugas
                                            </span>
                                        @endif
                                    ...
                                    @elseif($p->status == 'Dikembalikan')
                                        <span class="inline-flex items-center gap-1 bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg text-xs font-semibold">
                                            <i class="fa-solid fa-circle-check text-[10px]"></i> Kembali
                                        </span>
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    <div class="flex items-center justify-center gap-2 flex-wrap">
                                        ...
                                        @if ($p->status == 'Dikonfirmasi')
                                            <a href="{{ route('petugas.cetak.peminjaman', $p->id) }}" target="_blank"
                                        ...
                                        @if ($p->status == 'Menunggu Pengembalian')
                                            <form action="{{ route('petugas.konfirmasi_pengembalian', $p->id) }}" method="POST">
                                                @csrf
                                                <button class="inline-flex items-center gap-1 bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition shadow-sm">
                                        ...
                                        @if ($p->status == 'Ditolak Terlambat')
                                            <div class="flex flex-col gap-2">
                                                ...
                                                <form action="{{ route('petugas.konfirmasi_pengembalian_terlambat', $p->id) }}" method="POST">
                                                    @csrf
                                        ...
                                        @if ($p->status == 'Dikembalikan')
                                            <a href="{{ route('petugas.cetak.pengembalian', $p->id) }}" target="_blank"
