<div class="w-[270px] min-h-screen bg-[#14B8A6] text-white flex flex-col shadow-2xl relative overflow-hidden">
    ...
    <div class="relative z-10 px-6 py-7 border-b border-white/20">
        <div class="flex items-center gap-3">
            <div class="w-11 h-11 bg-white rounded-xl flex items-center justify-center shadow-lg shadow-black/10">
                <i class="fa-solid fa-book-open text-[#14B8A6] text-lg"></i>
            </div>
            <div>
                <h1 class="text-base font-bold tracking-wide">PERPUSTAKAAN</h1>
                <p class="text-[10px] text-white/70 font-medium tracking-widest">DIGITAL LIBRARY</p>
            </div>
        </div>
    </div>
    ...
    <nav class="relative z-10 flex-1 px-4 py-6 space-y-1.5">
        ...
        <a href="{{ route('user.dashboard') }}"
            class="sidebar-link {{ request()->routeIs('user.dashboard') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-white/90 hover:text-white hover:bg-white/15 transition-all duration-200">
        ...
        <a href="{{ route('user.favorites') }}"
            class="sidebar-link {{ request()->routeIs('user.favorites') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-white/90 hover:text-white hover:bg-white/15 transition-all duration-200">
            <div class="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center">
        ...
        <a href="{{ route('user.riwayat') }}"
            class="sidebar-link {{ request()->routeIs('user.riwayat') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-white/90 hover:text-white hover:bg-white/15 transition-all duration-200">
            <div class="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center">
        ...
        <a href="{{ route('user.ulasan.list') }}"
            class="sidebar-link {{ request()->routeIs('user.ulasan.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-white/90 hover:text-white hover:bg-white/15 transition-all duration-200">
            <div class="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center">
